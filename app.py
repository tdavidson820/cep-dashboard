# CEP Policy Intelligence Platform - ENHANCED Wisconsin Page
# Original homepage preserved + Enhanced Wisconsin state page with:
# - State executives section
# - Sortable county table (Dash DataTable)
# - All 11 columns from PDF
# - 24 NO CEP counties correctly displayed
...
COLORS = {
    ...
    'full_cep': '#34C759', 'partial_cep': '#FF9F0A', 'no_cep': '#FF3B30',
    'democrat': '#3B82F6', 'republican': '#EF4444'
}

# ====================
# STATE EXECUTIVES - NEW
# ====================
...
# ====================
# WISCONSIN DATA - CORRECTED WITH ALL 11 COLUMNS
# ====================

def load_wisconsin_data():
    """Load complete Wisconsin county data - 72 counties, all 11 columns, 24 NO CEP"""
    data = {
...
def load_new_jersey_data():
    """NJ data placeholder"""
    return pd.DataFrame({'County': ['Sample'], 'Population': [100000], 'Poverty': [15000], 'Eligible_Schools': [25], 'CEP_Schools': [10], 'Students_in_CEP': [5000], 'Status': ['PARTIAL CEP'], 'Coverage_Pct': [40], 'School_Gap': [15]})
...
# ====================
# (Keeping all original homepage components exactly as they were)
...
# ====================
# ENHANCED WISCONSIN STATE PAGE
# ====================

def create_state_executives_section(state_abbr):
    """NEW: State Leadership section with party colors"""
    ...
    for position, official in executives.items():
        party_color = COLORS['democrat'] if official['party'] == 'Democrat' else COLORS['republican']
        card = html.Div([html.Div(position, style={'fontSize': '12px', 'color': COLORS['text_secondary'], 'textTransform': 'uppercase', 'letterSpacing': '0.5px', 'fontWeight': '600', 'marginBottom': '8px'}), html.Div(official['name'], style={'fontSize': '18px', 'fontWeight': '600', 'color': COLORS['text_primary'], 'marginBottom': '6px'}), html.Div([html.Span('●', style={'color': party_color, 'marginRight': '6px', 'fontSize': '10px'}), html.Span(official['party'], style={'fontSize': '14px', 'color': party_color, 'fontWeight': '500'})])], style={'background': 'white', 'padding': '20px 24px', 'borderRadius': '8px', 'border': f'1px solid {COLORS["border"]}', 'minWidth': '200px'})
        cards.append(card)
...
def create_sortable_county_table(df):
    """NEW: Sortable Dash DataTable with all 11 columns"""
    columns = [
        {'name': 'County', 'id': 'County', 'type': 'text'},
        {'name': '2020 Census Total Population', 'id': 'Population', 'type': 'numeric', 'format': {'specifier': ','}},
        {'name': 'Children Under 18 in Poverty (2020)', 'id': 'Children_in_Poverty', 'type': 'numeric', 'format': {'specifier': ','}},
        {'name': 'School Districts in County', 'id': 'School_Districts', 'type': 'numeric'},
        {'name': 'CEP-Eligible Schools', 'id': 'Eligible_Schools', 'type': 'numeric'},
        {'name': 'CEP-Participating Schools', 'id': 'CEP_Schools', 'type': 'numeric'},
        {'name': 'Students Enrolled in CEP Schools', 'id': 'Students_in_CEP', 'type': 'numeric', 'format': {'specifier': ','}},
        {'name': '% of Eligible Schools Participating in CEP', 'id': 'Coverage_Pct', 'type': 'numeric', 'format': {'specifier': '.0f'}},
        {'name': 'School Gap', 'id': 'School_Gap', 'type': 'numeric'},
        {'name': 'CEP Coverage Status', 'id': 'Status', 'type': 'text'}
    ]
    
    return html.Div([html.H2("County Details", style={'fontSize': '32px', 'fontWeight': '600', 'color': COLORS['text_primary'], 'marginBottom': '16px'}), html.P("Click any column header to sort. Use the filter boxes to search.", style={'fontSize': '15px', 'color': COLORS['text_secondary'], 'marginBottom': '24px'}), dash_table.DataTable(id='county-table', columns=columns, data=df.to_dict('records'), sort_action='native', sort_mode='multi', filter_action='native', page_action='native', page_size=20, style_table={'overflowX': 'auto', 'border': f'1px solid {COLORS["border"]}', 'borderRadius': '12px', 'overflow': 'hidden'}, style_header={'backgroundColor': COLORS['off_white'], 'fontWeight': '600', 'fontSize': '13px', 'color': COLORS['text_secondary'], 'textTransform': 'uppercase', 'letterSpacing': '0.5px', 'padding': '16px 20px', 'borderBottom': f'2px solid {COLORS["border"]}', 'textAlign': 'left'}, style_cell={'padding': '16px 20px', 'fontSize': '15px', 'fontFamily': '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', 'textAlign': 'left', 'borderBottom': f'1px solid {COLORS["border"]}', 'whiteSpace': 'normal', 'height': 'auto'}, style_cell_conditional=[{'if': {'column_id': ['Population', 'Children_in_Poverty', 'Students_in_CEP']}, 'textAlign': 'right'}, {'if': {'column_id': ['School_Districts', 'Eligible_Schools', 'CEP_Schools', 'Coverage_Pct', 'School_Gap']}, 'textAlign': 'center'}, {'if': {'column_id': 'County'}, 'fontWeight': '500', 'minWidth': '150px'}], style_data_conditional=[{'if': {'filter_query': '{Status} = "FULL CEP"', 'column_id': 'Status'}, 'backgroundColor': COLORS['full_cep'], 'color': 'white', 'fontWeight': '500', 'borderRadius': '20px', 'textAlign': 'center', 'padding': '6px 14px'}, {'if': {'filter_query': '{Status} = "PARTIAL CEP"', 'column_id': 'Status'}, 'backgroundColor': COLORS['partial_cep'], 'color': 'white', 'fontWeight': '500', 'borderRadius': '20px', 'textAlign': 'center', 'padding': '6px 14px'}, {'if': {'filter_query': '{Status} = "NO CEP"', 'column_id': 'Status'}, 'backgroundColor': COLORS['no_cep'], 'color': 'white', 'fontWeight': '500', 'borderRadius': '20px', 'textAlign': 'center', 'padding': '6px 14px'}, {'if': {'row_index': 'odd'}, 'backgroundColor': COLORS['white']}, {'if': {'row_index': 'even'}, 'backgroundColor': COLORS['off_white']}], style_filter={'backgroundColor': COLORS['white'], 'border': f'1px solid {COLORS["border"]}', 'borderRadius': '4px', 'padding': '8px', 'fontSize': '14px'})], style={'maxWidth': '1400px', 'margin': '0 auto', 'padding': '0 40px 80px 40px'})

def create_county_map(df, fips_dict):
    df['FIPS'] = df['County'].map(fips_dict)
    fig = go.Figure(go.Choropleth(geojson="https://raw.githubusercontent.com/plotly/datasets/master/geojson-counties-fips.json", locations=df['FIPS'], z=df['CEP_Schools'], text=df['County'], colorscale=[[0, COLORS['no_cep']], [0.01, COLORS['partial_cep']], [1, COLORS['full_cep']]], marker_line_color='white', marker_line_width=1.5, colorbar=dict(title="<b>CEP Schools</b>", titlefont=dict(size=14, color=COLORS['text_primary']), tickfont=dict(size=12, color=COLORS['text_secondary']), thickness=15, len=0.7)))
